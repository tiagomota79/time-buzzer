...
const team1Info = document.querySelector('#team1info');
const team2Info = document.querySelector('#team2info');
...
team1NameInput.addEventListener('keydown', (ev) => {
  if (ev.key === 'Enter') {
    ...
    team1Info.innerHTML = `${team1Name} uses the 'q' key`;
  }
});
...
team2NameInput.addEventListener('keydown', (ev) => {
  if (ev.key === 'Enter') {
    ...
    team2Info.innerHTML = `${team2Name} uses the 'p' key`;
  }
});
